  ...
  return (
    <header className="absolute top-0 left-0 right-0 z-20 flex items-start justify-between px-[2.431vw] max-md:px-5 max-md:pt-5 max-md:items-center">
      <a
        ...
        className={`${metNav ? "mt-[2.431vw]" : "mt-[4.097vw]"} font-body font-medium text-[2.5vw] leading-[2.9vw] tracking-[-0.05vw] no-underline ${textColor} max-md:mt-0 max-md:text-[22px] max-md:leading-none`}
      >
        Wonen bij Weverskade
      </a>

      <div
        className={`${metNav ? "mt-[2.778vw]" : "mt-[5.069vw]"} flex items-center gap-[3.472vw] max-md:mt-0`}
      >
        {metNav ? (
        ...
        {ctaLabel && ctaHref ? (
          <a
            ...
            className={`flex items-center gap-[0.694vw] bg-green text-off-white no-underline rounded-full h-[2.847vw] ${metNav ? "pl-[2.014vw] pr-[1.806vw]" : "px-[1.597vw]"} max-md:h-auto max-md:px-4 max-md:py-2`}
          >
            {ctaArrow ? (
